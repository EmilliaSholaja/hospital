...
$patient_hospital_id = $patient_id;

// Fetch support tickets
$query = "
SELECT st.*, p.fullName AS patient_name, p.email AS patient_email 
FROM support_tickets st
JOIN patient p ON st.user_id = p.Patient_No
WHERE st.user_id = '$patient_hospital_id' 
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_ticket'])) {
    ...
    if (!empty($subject) && !empty($message)) {
        $insert_query = "INSERT INTO support_tickets (user_id, user_role, subject, message, status, created_at, fullName, email) 
                         SELECT '$patient_hospital_id', 'patient', '$subject', '$message', 'Open', NOW(), fullName, email 
                         FROM patient WHERE Patient_No = '$patient_hospital_id'";

        if (mysqli_query($connect, $insert_query)) {
            echo "<script>alert('Support ticket submitted successfully!'); window.location.href = '../patient/patient-support.php?tab_id={$tab_id}';</script>";
        } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send_response'])) {
    ...
    if (!empty($response)) {
        $update_query = "UPDATE support_tickets 
                         ...
                         WHERE ticket_id = '$ticket_id' AND user_id = '$patient_hospital_id'";

        if (mysqli_query($connect, $update_query)) {
            echo "<script>alert('Response sent successfully!'); window.location.href = '../patient/patient-support.php?tab_id={$tab_id}';</script>";
        } else {
